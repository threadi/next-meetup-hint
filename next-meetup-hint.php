...
/**
 * Initialize this plugin in the backend: load the events and get the event within the next configured days.
 *
 * @return void
 */
		...
		<table class="form-table">
			...
			<tr>
				...
				<td>
					...
					<p>
					<?php
						/* translators: %1$d will be replaced by a number for days. */
...
/**
 * Add the link to go to the user-specific settings in plugin list.
 *
...
function next_meetup_hint_add_settings(): void {
	...
	$permissions_tab_general_section = $permissions_tab->add_section( 'settings_section_permissions', 20 );
	...
	$field->set_description( __( 'Set amount days before the meetup to show the hint.', 'next-meetup-hint' ) );
	$field->set_min( 1 );
	$field->set_max( 365 );
	$field->set_step( 1 );
	...
	$field->set_description( __( 'Set amount days the hint would be hidden if user use the option to "Dismiss" the notice.', 'next-meetup-hint' ) );
	$field->set_min( 0 );
	$field->set_max( 365 );
	$field->set_step( 1 );
	$setting->set_field( $field );
	...
	$setting->set_field(
		array(
			...
			'description' => __( 'If activated, a link to GoogleMaps is displayed for each event, if geo-coordinates are available for the event. This allows you to quickly see where the Meetup is taking place and find your way there.', 'next-meetup-hint' ),
		)
	);
	...
	$setting->set_field(
		array(
			...
			'description' => __( 'If activated, a link to OpenStreetMap is displayed for each event, if geo-coordinates are available for the event. This allows you to quickly see where the Meetup is taking place and find your way there.', 'next-meetup-hint' ),
		)
	);
	...
	$setting->set_field(
		array(
			...
			'description' => __( 'If activated, a link to BingMaps is displayed for each event, if geo-coordinates are available for the event. This allows you to quickly see where the Meetup is taking place and find your way there.', 'next-meetup-hint' ),
		)
	);
	...
	if ( defined( 'NEXT_MEETUP_HINT_ACTIVATION' ) ) {
...
function next_meetup_hint_is_user_allowed( int $user_id ): bool {
	...
	// bail if user has role, that is not in list of allowed roles.
	$hide          = true;
...
/**
 * Show GoogleMap link on the event.
 *
...
/**
 * Show OpenStreetMap link on the event.
 *
 * @param array<string,array<string>> $event The event data.
 *
 * @return void
	...
	<div id="nmhwhy" class="overlay">
		<div class="popup">
			...
			<div class="content">
				<?php
				/* translators: %1$s will be replaced by a URL. */
				echo '<p>' . wp_kses_post( sprintf( __( 'This hint is displayed by the WordPress plugin <a href="%1$s" target="_blank">Next Meetup Hint (opens a new window)</a>. It will show you the Meetup closest to you. The basis for the data is contained in WordPress itself and your WordPress user.', 'next-meetup-hint' ), 'https://wordpress.org/plugins/next-meetup-hint/' ) ) . '</p>';
				/* translators: %1$s will be replaced by a URL. */
				...
				/* translators: %1$s will be replaced by a URL. */
				echo '<p>' . wp_kses_post( sprintf( __( 'If you have any further questions, please feel free to contact <a href="%1$s" target="_blank">our support forum (opens a new window)</a>.', 'next-meetup-hint' ), 'https://wordpress.org/support/plugin/next-meetup-hint/' ) ) . '</p>';
				?>
			</div>
		</div>
	</div>
...
/**
 * Run check for the active version to compile update tasks if necessary.
 *
 * @return void
 */
function next_meetup_hint_check_version(): void {
	...
	// get the database version (version, which was last installed).
...
function next_meetup_hint_activation(): void {
	...
	define( 'NEXT_MEETUP_HINT_ACTIVATION', 1 );
...
/**
 * Show ICS-download-link on the event.
 *
 * @return void
 */
